...
public class InventorySystem {
    ...
    public static void removeUnit(Product product) {
        for(Map.Entry<Location, Unit> entry: locationMap.entrySet()){
            if(entry.getValue()!=null && Objects.equals(product.id, entry.getValue().productId))
...
class User {
    public void addProduct(){
        InventorySystem.addProduct(new Product("", (double) 0,"",0,Size.L));
    }
